    ...
    <div>
        <div class="space-y-3">
                <div class="flex items-center w-full justify-between">
                    ...
                    <input v-model="directorname" :="directornameProps" @keyup.enter="createDirector" type="text" class="w-[280px] h-[40px] border-none bg-black ring-[3px] ring-[rgb(0,137,208,0.5)] text-[24px] text-center rounded-lg" />
                    <div @click="createDirector" class="cursor-pointer content-center w-[80px] h-[40px] bg-[#0089D0] text-[24px] text-center text-white rounded-lg">Add</div>
                </div>
                <p class="text-red-500 text-center text-sm">{{ errors.directorname  }}</p>
...
function required(value) {
  return value ? true : 'Director name is required';
}
